...
class BuildingMart4 extends Building{
...
ExitLocation.x = 52;ExitLocation.y = 24;
MapDoorLocation.x = 52;MapDoorLocation.y = 23;
...
override public function getNextLine()
{
  ...
  if(currentTalkingCharacter.ID == 1)
  {
    if(currentTalkingCharacter.TalkedTo >= 0)
    {
      ...
      {
	Text.htmlText = "<font size='16'>Stock up, this is it!</font>";
      }
...
override public function initializeCharacters()
{
 ...
 var Merch = new Character
  ...
  private function initBuyableItems()
  {
    ...
    BuyableItems.add(item);
    var item = new Item();
    item.updateItem(new Point(),"Full Heal");
    BuyableItems.add(item);
    var item = new Item();
    item.updateItem(new Point(),"Full Heal Group");
    BuyableItems.add(item);
    var item = new Item();
    item.updateItem(new Point(),"MP Restore");
    BuyableItems.add(item);
    var item = new Item();
    item.updateItem(new Point(),"MP Restore Group");
    BuyableItems.add(item);
    var item = new Item();
    item.updateItem(new Point(),"MP Full Restore");
    BuyableItems.add(item);
    var item = new Item();
    item.updateItem(new Point(),"MP Full Restore Group");
    BuyableItems.add(item);
    var item = new Item();
    item.updateItem(new Point(),"Full Restore");
    BuyableItems.add(item);
    var item = new Item();
    item.updateItem(new Point(),"Full Restore Group");
    BuyableItems.add(item);
